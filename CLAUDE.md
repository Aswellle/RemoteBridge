...
```sh
pnpm --filter @remotebridge/shared build   # required before standalone typecheck/test; `pnpm dev` handles this automatically (turbo dependsOn ^build)
...
`ws/connection-registry.ts` is the sole owner of room state (the `hostSockets`/`clientSockets`/`sessionRooms` Maps are private to this module) — it provides registration (`registerHost`/`unregisterHost`/`registerClient`/`unregisterClient`, each returning a boolean used as a reconnect-race guard: only remove if the registered socket still matches the caller's), lookups (`getHostSocket`/`getClientSocket`/`isHostOnline`/`isClientOnline`/`getClientHost`/`getHostClients`/`getRoomInfo`), iteration (`forEachHost`/`forEachClient`/`forEachClientOfHost`), and room rebinding for host-reconnect (`rebindClientToHost`, `clearHostClients`, `clearAll`). Per-connection metadata (`ConnectionMeta`) is stored in a module-level `WeakMap<WebSocket, ConnectionMeta>` and accessed via the exported `getConnMeta(ws)`/`setConnMeta(ws, meta)` accessors — never via `(ws as any).__meta` (that pattern was removed).
