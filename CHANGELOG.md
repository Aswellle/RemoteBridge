...
- **CI 在 Node 20 下失败**：`local-relay.ts` 的端口探测使用了 `Promise.withResolvers`，
  ...
  导致 `local-relay-probe` 4 个用例全挂（失败同时把 `starting` 标志永久置位，连带
  后续启动请求全部被"正在启动中"挡掉）。改为通用 `new Promise` 写法，并把 `starting`
  的复位移入 `finally`；`tsconfig` 中为它引入的 `ES2024.Promise` lib 一并回退，
  使该 API 在未来无法再通过类型检查
- **更新检查失败横幅长期驻留**：一次网络抖动（`net::ERR_CONNECTION_CLOSED`）会被广播成
  全局 error 状态，顶部横幅此后一直显示"检查更新失败"并遮挡正常操作。现在检查/下载
  失败不再进入全局状态（错误经 IPC 返回值在"关于"页内联展示），`UpdateStatus` 类型中
  直接移除 `error` 变体，使"错误横幅"在类型层面不可表达
- **更新横幅可见性规则**：顶部横幅只承载需要用户动作的三态（发现新版本 / 下载中 / 已就绪），
  带"下载更新""立即安装"按钮与最右侧可关闭的 ✕；关闭状态按版本号记忆，
  同一版本不再重复弹出，出现更新的版本时重新提示
- **下载失败卡在进度条**：下载中途失败会把状态退回"可下载"（横幅恢复可操作），
  原先会永久停在进度条上
- **自动检查时机**：仅在应用启动时自动检查一次（主进程 10s 延迟检查）；
  进入"关于"页不再触发检查（实测进入页面 0 次调用），改为读取当前已知状态，
  用户可点"检查更新"主动核对并看到有无新版本

- **操作性按钮可见度**（设置/共享目录/消息中心/已连接客户端）：静息态可见性系统性不足
  - `Button` 基类原先统一 `disabled:opacity-40`，实测禁用态标签低至 **1.32:1**（亮色，primary），
    文字几乎不可读；改为各变体显式给出禁用态 surface + 文字色（实测 8.97:1 / 6.69:1），
    并新增 `outline` 变体（可见描边 + surface，用于刷新等次级操作）
  - `Input`：占位符不再叠加 `/60` 透明度，禁用态不再整体降透明度而是保留可读文字色 +
    可见描边 —— 占位符常承载操作指引（"请先选择一个客户端"），实测由 **1.66:1 → 7.76:1**（暗色 1.69 → 5.00）
  - `IconButton`：静息态改为可见 surface（原先透明，行内"编辑别名/移除目录"未悬浮时不可辨识），
    新增 `tone="danger"`
  - 共享目录：`清空目录` 由 15% 红底改为可见红描边（实测 4.29 → **5.48:1**，暗色 4.03 → 4.63），
    行内图标按钮改为可见描边按钮、别名文字改用 `secondary-foreground`（14.44:1 / 13.1:1）
  - 已连接客户端：视图切换改为分段控件（当前视图以 surface + 强调色文字 + 图标标出，替代"文字前加 ● "），
    切换控件按 ARIA APG tabs 模式实现 roving tabindex + 左右方向键/Home/End；刷新按钮改用 `outline`
  - 关于页：更新错误由 `max-w-48 truncate`（挤压后无法阅读）改为整行横幅，完整展示不截断，
    超长时由 `title` 悬浮查看；顶部 Toast 的截断消息同样补 `title`
  - 新增语义 token `--danger-text`：`--destructive` 同时承担填充与文字两种角色，作为文字落在
    10% 红底上仅 **3.74:1**（暗色），文字角色单独取值后 5.50:1（亮色 5.55:1）；
    暗色 `--muted-foreground` 由 62% 提到 66%（surface-subtle 上 4.40 → 5.00）
