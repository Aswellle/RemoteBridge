...
- **裸机 systemd 部署**：`deploy/systemd/remotebridge-server.service` 设置了
  ...
  ```
- **`scripts/deploy-server.sh` 仅构建产物**：只做 `pnpm build`（shared + server），
  **不启动服务器**，也无进程监管。构建完成后需另行通过 systemd 或 `docker compose up -d`
  启动服务（脚本末尾有提示），生产环境务必使用这两种方式之一。
...
- **收紧限流**：将 `.env`/`docker-compose.yml` 中的 `RL_REGISTER_MAX` 调小后重启 relay
  —— 该变量由 `packages/shared/src/security.ts` 的 `RATE_LIMIT_CONFIG.REGISTER_HOST_MAX`
  读取（默认 5 次/分钟），仅影响 `register-host` 端点。注意：`.env.example`/
  `docker-compose.yml` 中仍声明有 `RATE_LIMIT_MAX`/`RATE_LIMIT_WINDOW`，但服务端并未消费，
  修改它们**不会**生效。
...
- **数据层手动封禁**：`hosts` 表已有 `is_banned` 列，且 `/auth/connect` 的候选主机查询会过滤
  `is_banned = 0`（`apps/server/src/routes/auth.ts:243`）。
...
截至目前（见 `CHANGELOG.md`），workspace 包的版本并不统一：`apps/desktop` 在 `1.3.11`，
而 `apps/server`、`apps/web`、`packages/shared` 仍锁定在 `1.0.0`，且**没有 git tag/release
流程**。
