...
This room state (`hostSockets`, `clientSockets`, `sessionRooms`, `hostClients`) is currently four in-memory `Map`s owned by `apps/server/src/ws/connection-registry.ts` (see "Implementation" below) — the relay itself stores no files and persists no room state to disk.
...
`hostSockets`/`clientSockets`/`sessionRooms`/`hostClients` Maps and `ConnectionMeta` are
private to this module. It exposes registration (`registerHost`/`unregisterHost`/
`registerClient`/`unregisterClient`, each returning a boolean used as a reconnect-race guard:
only remove if the registered socket still matches the caller's), lookups (`getHostSocket`/
